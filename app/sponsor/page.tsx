...
import { ArrowLeft, Heart, Target, Users, BookOpen, GraduationCap, Handshake, Mail } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#0a0a0f] via-[#0d1b2a] to-[#1b3a5c] text-white overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        ...
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-orange-500/10 rounded-full blur-3xl animate-pulse-slow delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-yellow-500/5 rounded-full blur-3xl animate-pulse-slow delay-2000"></div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-16 sm:py-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-blue-500/10 p-8 md:p-12">
              ...
              <div className="space-y-6 text-white/70 leading-relaxed">
                <p>
                  <strong className="text-white">Changing The Mindset Of An African Child</strong> is more 
                  than just a textbook—it's a movement to make education accessible 
                  in isiZulu.
                </p>
                
                <div className="p-4 bg-white/5 border border-white/10 rounded-xl">
                  <p className="text-white/60 text-sm">
                    <span className="text-amber-400 font-semibold">🤝 Your sponsorship can help:</span>
                  </p>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    { text: "Print physical copies for schools in need", icon: "📚" },
                    { text: "Distribute books to rural communities", icon: "🌍" },
                    { text: "Support indigenous language education", icon: "🗣️" },
                    { text: "Empower the next generation of African learners", icon: "🌟" },
                  ].map((item) => (
                    <div key={item.text} className="flex items-start gap-3 p-3 bg-white/5 border border-white/5 rounded-xl hover:bg-white/10 transition-all group">
                      <span className="text-amber-400 group-hover:scale-110 transition-transform flex-shrink-0">{item.icon}</span>
                      <span className="text-sm text-white/60 group-hover:text-white/80 transition-colors">{item.text}</span>
                    </div>
                  ))}
                </div>
                
                <div className="p-4 bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-yellow-500/10 border border-amber-500/20 rounded-xl">
                  <p className="font-semibold text-amber-400">🇿🇦 Join the Mission</p>
                  <p className="text-sm text-white/60 mt-1">
                    Together, we can change the mindset of an African child.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <Link
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-semibold hover:shadow-xl hover:shadow-amber-500/40 transition-all hover:scale-105"
                >
                  <Mail size={18} />
          ...
          <div className="space-y-4">
            {/* Why Sponsor - Removed Impact Goals */}
            <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-blue-500/10 p-6">
              <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-4">Why Sponsor?</h3>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-2 rounded-lg">
                  <GraduationCap size={16} className="text-amber-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-sm text-white/80">Empower Education</p>
                    <p className="text-xs text-white/30">Make learning accessible in isiZulu</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-2 rounded-lg">
                  <Users size={16} className="text-orange-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-sm text-white/80">Community Impact</p>
                    <p className="text-xs text-white/30">Reach rural communities across SA</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-2 rounded-lg">
                  <Handshake size={16} className="text-yellow-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-sm text-white/80">Partnership</p>
                    <p className="text-xs text-white/30">Join a movement for change</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Quote */}
            <div className="bg-gradient-to-br from-amber-500/10 via-orange-500/10 to-yellow-500/10 border border-white/10 rounded-2xl p-6 shadow-2xl shadow-amber-500/10">
              <p className="text-sm text-white/40 italic">
                "Together, we can change the mindset of an African child."
              </p>
              <p className="text-xs text-white/20 mt-2">— Author's Vision</p>
            </div>
          </div>
        </div>
